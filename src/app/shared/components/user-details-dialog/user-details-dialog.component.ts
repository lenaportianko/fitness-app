...
import { CountryResponse } from '../../../modules/auth/models/country-response.model';
import { map, Observable, startWith } from 'rxjs';
import { CountryData } from '../../../modules/auth/models/country-data.model';
...
export class UserDetailsDialogComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit(): void {
    this.userForm = new FormGroup({
      ...
      age: new FormControl(this.data.age, Validators.min(18)),
      height: new FormControl(this.data.height, Validators.min(100)),
      weight: new FormControl(this.data.weight, Validators.min(40)),
      weightGoal: new FormControl(this.data.weightGoal, Validators.min(40))
    });
    ...
    this.filteredCountryOptions = this.userForm.controls['country'].valueChanges.pipe(
      startWith(''),
  ...
  private filter(value: string): string[] {
    ...
    if (!filteredValues.length) {
      this.userForm.controls['country'].setErrors({ 'wrong-value': true });
    }

    return filteredValues;
  }

  public isExistAvatar(): boolean {
    return this.userForm.controls['avatar'].value ?? false;
  }

  public removeAvatar(): void {
    this.userForm.controls['avatar'].setValue('');
  }

  public onChangeFileInput(eventTarget: EventTarget | null): void {
    const selectedFile: FileList | null = (eventTarget as HTMLInputElement).files;
    ...
    if (selectedFile) {
      ...
      reader.onload = (event: ProgressEvent<FileReader>): void => {
        if (event.target) {
          this.userForm.controls['avatar'].setValue(event.target.result);
        }
      };
